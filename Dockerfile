...
# ЕДИНСТВЕННОЕ ЗДЕСЬ, ЧТО НЕ ПРИШПИЛЕНО: эндпоинт отдаёт только новейший и
# параметр версии молча игнорирует - проверено, с ?version=0.5.60 приходит тот
# же редирект на 0.5.69. Две сборки в разные дни несут разный ntk, и сделать с
# этим отсюда нечего. Появится версионный адрес - пин станет на место.
RUN a="$(cat /tmp/arch)"; \
